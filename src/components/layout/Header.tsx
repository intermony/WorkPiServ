...
import { isMobileDevice, waitForPiSDK } from '@/lib/pi';
...
export function Header() {
  ...
  const handleLoginClick = async () => {
    // Ordinateur : le Pi Browser n'existe pas sur desktop -> QR direct
    if (!isMobileDevice()) { setShowPiModal(true); return; }
    // Mobile : laisser au script SDK le temps d'arriver (réseau lent)
    const ready = await waitForPiSDK(5000);
    if (!ready) { setShowPiModal(true); return; }

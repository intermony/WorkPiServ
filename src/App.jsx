...
// ==================== PC / GUEST SPONSOR BANNER ====================
const PiBrowserNotification = () => {
  const [isPC, setIsPC] = useState(false);

  useEffect(() => {
    // Détecte si l'application tourne en dehors du Pi Browser
    const isPiBrowser = navigator.userAgent.includes("PiBrowser") || (window.Pi && window.Pi.initialized);
    if (!isPiBrowser) {
      setIsPC(true);
    }
  }, []);
  ...
  return (
    <div style={{
      ...
      <span style={{ marginRight: '10px' }}>💡 <strong>Mode Démo (PC) :</strong> Pour finaliser des contrats et payer de vrais services, ouvrez ce site dans le <strong>Pi Browser</strong> sur mobile.</span>
      <span style={{ display: 'inline-block', color: '#cbd5e1' }}>
        Pi est une nouvelle cryptomonnaie développée par des docteurs en Science Informatique de Stanford, avec plus de 55 millions de membres à travers le monde. Pour réclamer vos Pi, suivez ce lien 
        <a href="https://minepi.com/alibentaher" target="_blank" rel="noopener noreferrer" style={{ color: '#c084fc', fontWeight: 'bold', margin: '0 5px', textDecoration: 'underline' }}>
          https://minepi.com/alibentaher
        </a> 
        et utilisez mon pseudo <strong>(alibentaher)</strong> comme code d'invitation.
      </span>
    </div>
  );
};
...
const StatusBadge = ({ status, T }) => {
  const map = {
    pending: T.statusPending || 'En attente de paiement', active: T.statusActive,
...
const OrderDetailPage = ({ user, addToast, T }) => {
  ...
  // Déclenchement du vrai paiement via la blockchain Pi
  const handleBlockchainPayment = async () => {
    if (!window.Pi) {
      addToast("Le SDK Pi n'est pas chargé. Ouvrez cette page dans le Pi Browser.", "error");
      return;
    }
    setIsPaying(true);
    try {
      window.Pi.createPayment({
        amount: order.price,
        memo: `Paiement Escrow - Commande #${order._id?.toString().slice(-4)}`,
        metadata: { orderId: order._id }
      }, {
        onReadyForServerApproval: async (paymentId) => {
          // Étape 1 : Envoi au backend pour approbation de la transaction
          await api.post('/api/payments/approve', { paymentId });
        },
        onReadyForServerCompletion: async (paymentId, txid) => {
          // Étape 2 : Signature de l'utilisateur effectuée, finalisation backend
          await api.post('/api/payments/complete', { paymentId, txid });
          addToast("Paiement validé avec succès sur la Blockchain Pi !", "success");
        ...
        onCancel: (paymentId) => {
          addToast("Paiement annulé par l'utilisateur.", "warning");
          setIsPaying(false);
        },
        onError: (error, paymentId) => {
          addToast("Une erreur est survenue lors du paiement Pi.", "error");
          console.error(error);
          setIsPaying(false);
        }
      });
    } catch (err) {
      addToast("Erreur d'initialisation du protocole Pi.", "error");
      setIsPaying(false);
    }
  };
    ...
    <div className="animate-fade">
      ...
      <Card>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))',gap:'1rem',marginBottom:'1rem'}}>
          ...
          <div><p style={{color:'#64748b',marginBottom:'0.5rem'}}>{T.labelEscrow}</p><p style={{fontWeight:700,color:'#f59e0b'}}>{order.escrowAmount || order.price} π</p></div>
        ...
        {/* AJOUT DU BOUTON DE PAIEMENT SI LA COMMANDE EST EN ATTENTE */}
        {order.status === 'pending' && isClient && (
          <div style={{ marginTop: '1.5rem', padding: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
            <Button 
              ...
              disabled={isPaying}
              style={{ width: '100%', maxWidth: '400px', padding: '1rem', fontSize: '1.1rem', background: 'linear-gradient(135deg, #f59e0b, #d97706)', fontWeight: 'bold' }}
            >
              {isPaying ? 'Transaction en cours...' : `💳 Confirmer et Payer ${order.price} π`}
            </Button>
          </div>
        )}
...
function App() {
  ...
  return (
    <div className="app-layout">
      {/* Ajout du bandeau de parrainage et d'alerte mode démo PC au sommet du layout */}
      <PiBrowserNotification />
